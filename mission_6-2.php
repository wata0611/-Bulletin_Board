<html>
	...
	<body>
		...
		<?php
			$dsn = 'mysql:dbname=tb210046db;host=localhost';
			$user = 'tb-210046';
			$password = 'ntvfMmXz38';
			$pdo = new PDO($dsn, $user, $password, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING));
			$sql = "CREATE TABLE IF NOT EXISTS tbt2"
			." ("
		...
		<p>旅行先/おすすめの観光地など：<input type="text" name="comment" value="<?php if(isset($edi_comment)){echo $edi_comment;}else{echo "/";}?>"></p>
		...
		<?php
			if(!empty($_POST["delete"]) && !empty($_POST["del_num"]) && !empty($_POST["del_pass"])){
				...
				if($del_error == false){
					$sql = 'delete from tbt2 where id=:id';
			...
			if(!empty($_POST["comment"])&&!empty($_POST["name"])&&!empty($_POST["sub_name"])){
				if(!empty($_POST["edi_num_store"])){
					...
					$sql = 'update tbt2 set name=:name,comment=:comment,time=:time,pass=:pass where id=:id';
					$stmt = $pdo->prepare($sql);
					...
					$sql = $pdo -> prepare("INSERT INTO tbt2 (name, comment, time, pass) VALUES (:name, :comment, :time, :pass)");
